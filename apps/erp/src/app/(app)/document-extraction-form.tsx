  ...
  return (
    <div className="grid gap-4 rounded-lg border border-line bg-surface p-4">
      ...
      <div className="overflow-hidden rounded-lg border border-line">
        <div className="border-b border-line bg-surface-strong px-3 py-2 text-sm font-semibold text-foreground">
          {extractionCopy.lineItemsTitle}
        </div>
        <div className="divide-y divide-line">
          {extraction.lineItems.map((item, index) => (
            <div
              className="grid gap-2 px-3 py-2 text-sm md:grid-cols-[1fr_100px_140px]"
      ...
      <details className="rounded-lg border border-line bg-surface-strong">
        ...
        <pre className="max-h-72 overflow-auto border-t border-line bg-slate-950 p-4 text-xs leading-5 text-slate-100">
  ...
  return (
    ...
    >
      ...
      <label className="block">
        ...
        <textarea
          className="min-h-32 w-full resize-y rounded-lg border border-line bg-surface px-3 py-2 text-sm text-foreground outline-none transition focus:border-slate-400"
          maxLength={12000}
          minLength={20}
          name="documentText"
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div
          className={`rounded-lg border px-3 py-2 text-sm ${getStateClassName(state.status)}`}
          role="status"
        >
          {state.message}
        </div>
