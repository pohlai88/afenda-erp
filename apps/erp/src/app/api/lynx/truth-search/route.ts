...
const truthSearchRequestSchema = z.object({
  query: z.string().trim().min(1).max(800),
});

const truthSearchUiRequestSchema = z.object({
...
function extractLatestQueryFromUiMessages(messages: unknown[]): string | null {
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const parsed = truthSearchRequestSchema.safeParse(body);
    const uiParsed = parsed.success
      ? null
      : truthSearchUiRequestSchema.safeParse(body);
    ...
    if (parsed.success) {
      ...
      query = extractLatestQueryFromUiMessages(uiParsed.data.messages);
    }
