...
import { hasDocumentSensitiveReadAccess, hasDocumentWriteAccess } from "@afenda/auth";
import { isOrganizationDocumentLegalHoldActive } from "@afenda/db";
import { loadModuleWorkspaceSession } from "@/routes/workspace/shared/workspace-route-cache";
import {
  BulletColumns,
  ModuleLinkGrid,
...
import { DocumentExtractionForm } from "@/routes/workspace/shared/document-extraction-form";
import { DocumentUploadForm } from "@/routes/workspace/shared/document-upload-form";
import { SystemAdminDocumentRegistryTrailingCell } from "@afenda/feature-system-admin/client";
...
export async function ModuleScreenDocumentsSection({
  ...
  const { metadata, workspace, moduleQuery, organization } =
    await loadModuleWorkspaceSession({ moduleId, searchParams });
  const blobEnv = getBlobEnv();
  const surfaceKeys = metadata.getListSurfaceKeys();
  const canViewSensitive = hasDocumentSensitiveReadAccess(
    organization.capabilities,
    moduleId,
  );
  const canWrite = hasDocumentWriteAccess(organization.capabilities, moduleId);
  const organizationLegalHoldActive = await isOrganizationDocumentLegalHoldActive(
    organization.id,
  );
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-surface-xl">
      <GovernedPatternCListSection
      ...
      <GovernedPatternCListSection
        ...
        description="Upload, download, retention, and governance events for this module."
        surfaceKey={surfaceKeys.documentActivity}
        listConfiguration={metadata.buildDocumentActivityLinesListSurface({
          events: workspace.documentActivityEvents,
          window: workspace.documentActivityWindow,
          query: moduleQuery,
        })}
        parentAccessAllowed
        layout="embedded"
