# ARCH-002 · ERP Domain Package Architecture
...
`packages/domain` is the shared contract layer, not the long-term home for
finance, HR, sales, inventory, or CRM business logic.
...
metadata, components, or data access.
...
`packages/domain` owns cross-module contracts only:
...
`packages/domain` depends on `@afenda/governed-surface` for list-surface builder
types today. After extraction, feature packages should own module builders;
`packages/domain` should keep only shared types and registry contracts.

## Cross-module dependency rules

- Prefer shared contracts in `@afenda/domain` over feature-to-feature imports.
...
- Allow direct feature-to-feature imports only for stable, documented seams.
  Avoid cycles; extract shared contracts to `@afenda/domain` when needed.
...
3. Move command/query services, list builders, components, and tests out of
   `packages/domain`.
4. Leave registry IDs, navigation contracts, and shared types in `packages/domain`.
...
`@afenda/ai`, `@afenda/auth`, `@afenda/config`, `@afenda/db`, `@afenda/domain`,
