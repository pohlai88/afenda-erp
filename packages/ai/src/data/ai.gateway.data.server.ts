import {
  ...
  hasAiGatewayRuntimeCredentials as configHasAiGatewayRuntimeCredentials,
  resolveAiGatewayReportApiKey,
} from "@afenda/config/env";
import { APICallError, createGateway, gateway, type RerankingModel } from "ai";
import type {
  GatewaySpendReport,
...
function normalizeModelIds(modelIds: readonly (string | undefined)[]) {
  return [
    ...new Set(
      modelIds
...
export function getConfiguredAiGatewayModelIds(
  ...
  return normalizeModelIds([
    ...
    env.AFENDA_AI_MODEL,
    env.AFENDA_AI_FAST_MODEL,
    env.AFENDA_AI_HIGH_CONFIDENCE_MODEL,
    env.RERANK_MODEL,
    ...additionalModelIds,
  ]);
}
...
export function hasAiGatewayRuntimeCredentials(
  ...
  return configHasAiGatewayRuntimeCredentials(input);
}
