...
import { tool } from "ai";
import { scoreAiConfidence } from "../policies/ai.confidence.policy";
import { assembleAiContext } from "../data/ai.context-builder.data";
import { assertCapabilityAllowed } from "../policies/ai.guardrails.policy";
import {
...
type InspectionResult = {
  ...
  workspace: ErpAssistantToolWorkspace;
  stats: ErpAssistantToolWorkspaceStats;
  evidence: readonly ReturnType<typeof evidenceRecordSchema.parse>[];
};

function createRootCauseEntry(input: {
  id: string;
  title: string;
...
}): ReturnType<typeof rootCauseAnalysisSchema.parse> {
  return rootCauseAnalysisSchema.parse({
    id: input.id,
    title: input.title,
    moduleId: resolveModuleIdFromEvidence(
      input.evidence,
      input.fallbackModuleId,
    ),
    ...
    confidenceBreakdown: scoreAiConfidence({
      ...
      directSourceCount: input.evidence.length,
      missingDataCount: input.missingData.length,
...
function mapInspectionsToRootCauses(
  inspections: readonly InspectionResult[],
  mapper: (
    inspection: InspectionResult,
    index: number,
  ) => Parameters<typeof createRootCauseEntry>[0],
) {
  return inspections.map((inspection, index) =>
    createRootCauseEntry(mapper(inspection, index)),
  );
}
  ...
  return {
    analyzeProfitAndLoss: tool({
      ...
      execute: async (problem) => {
        ...
        return mapInspectionsToRootCauses(
          inspected.slice(0, 6),
