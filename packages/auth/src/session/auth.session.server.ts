...
import { readOrganizationOperatingContextLabels } from "../contracts/auth.operating-context";
import {
...
} from "./auth.session-serialization.server";
...
export function createDevSessionCookie(session: UserSession) {
  return serializeDevSessionCookie(session);
}
...
async function getSessionFromNeonAuth() {
  ...
  try {
    ...
    logServerEvent(
      "error",
      "Failed to hydrate Neon session from tenant database.",
      {
        module: "auth",
        operation: "session.hydrate",
      },
      {
        error,
        userId: user.id,
...
async function loadSession(): Promise<UserSession | null> {
  ...
  const devSession = value ? parseDevSessionCookie(value) : null;
