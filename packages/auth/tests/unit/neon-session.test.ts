...
vi.mock("server-only", () => ({}));
vi.mock("../../src/neon-auth/runtime/neon-auth.server", () => ({
...
import {
  getNeonAuthServer,
  isNeonAuthReady,
} from "../../src/neon-auth/runtime/neon-auth.server";
import { readNeonAuthSessionPayload } from "../../src/neon-auth/runtime/neon-session.server";

describe("neon session payload", () => {
  ...
  it("hydrates session payload from the Neon server SDK", async () => {
  ...
  it("does not fall back to manual cookie parsing when the SDK has no session", async () => {
    vi.mocked(isNeonAuthReady).mockReturnValue(true);
    vi.mocked(getNeonAuthServer).mockReturnValue({
