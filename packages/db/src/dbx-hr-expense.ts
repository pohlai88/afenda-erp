...
export const hrExpenseClaims = pgTable("hr_expense_claims", {
    ...
    uniqueIndex("hr_expense_claims_org_number_uidx").on(table.organizationId, table.claimNumber),
    index("hr_expense_claims_org_status_idx").on(table.organizationId, table.claimStatus),
    index("hr_expense_claims_org_employee_idx").on(table.organizationId, table.employeeId),
    index("hr_expense_claims_org_category_idx").on(table.organizationId, table.categoryCode),
    index("hr_expense_claims_org_department_idx").on(table.organizationId, table.departmentId),
    index("hr_expense_claims_org_cost_center_idx").on(table.organizationId, table.costCenterCode),
    index("hr_expense_claims_org_project_idx").on(table.organizationId, table.projectCode),
    index("hr_expense_claims_org_submitted_idx").on(table.organizationId, table.submittedAt),
    index("hr_expense_claims_org_duplicate_probe_idx").on(table.organizationId, table.employeeId, table.primaryExpenseDate, table.claimAmount, table.merchantName),
]);
...
export const hrExpensePolicies = pgTable("hr_expense_policies", {
    ...
    uniqueIndex("hr_expense_policies_org_group_uidx").on(table.organizationId, table.policyGroupCode),
    index("hr_expense_policies_org_active_idx").on(table.organizationId, table.active),
]);
/** HRM-EXP-014 — approval routing rules per policy group. */
export const hrExpenseApprovalRoutes = pgTable("hr_expense_approval_routes", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    policyGroupCode: text("policy_group_code").notNull().default("default"),
    ...
    index("hr_expense_approval_routes_org_group_idx").on(table.organizationId, table.policyGroupCode),
    index("hr_expense_approval_routes_org_priority_idx").on(table.organizationId, table.priority),
]);
/** HRM-EXP-015 — per-claim approval workflow snapshot. */
export const hrExpenseApprovals = pgTable("hr_expense_approvals", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    ...
    uniqueIndex("hr_expense_approvals_org_claim_uidx").on(table.organizationId, table.claimId),
    index("hr_expense_approvals_org_status_idx").on(table.organizationId, table.status),
]);
/** HRM-EXP-020 — policy exceptions (over-limit, late, missing receipt). */
export const hrExpenseExceptions = pgTable("hr_expense_exceptions", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    claimId: text("claim_id")
        .notNull()
    ...
    index("hr_expense_exceptions_org_claim_idx").on(table.organizationId, table.claimId),
    index("hr_expense_exceptions_org_status_idx").on(table.organizationId, table.status),
    uniqueIndex("hr_expense_exceptions_org_claim_kind_uidx").on(table.organizationId, table.claimId, table.kind),
]);
/** HRM-EXP-004/008 — per-category limits and mandatory receipt flags. */
export const hrExpensePolicyCategoryRules = pgTable("hr_expense_policy_category_rules", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    ...
    uniqueIndex("hr_expense_policy_category_rules_policy_cat_uidx").on(table.policyId, table.categoryCode),
    index("hr_expense_policy_category_rules_org_policy_idx").on(table.organizationId, table.policyId),
]);
/** HRM-EXP-007 — eligibility matrix by employee attributes and category. */
export const hrExpenseEligibilityRules = pgTable("hr_expense_eligibility_rules", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    policyGroupCode: text("policy_group_code").notNull().default("default"),
    categoryCode: text("category_code"),
    ...
    index("hr_expense_eligibility_rules_org_group_cat_idx").on(table.organizationId, table.policyGroupCode, table.categoryCode),
    index("hr_expense_eligibility_rules_org_scope_idx").on(table.organizationId, table.legalEntityCode, table.workLocationCode, table.departmentId),
]);
/** HRM-EXP-003 — receipt / invoice attachments linked to claims. */
export const hrExpenseClaimReceipts = pgTable("hr_expense_claim_receipts", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    claimId: text("claim_id")
        .notNull()
    ...
    index("hr_expense_claim_receipts_org_claim_idx").on(table.organizationId, table.claimId),
    index("hr_expense_claim_receipts_org_fingerprint_idx").on(table.organizationId, table.receiptFingerprint),
]);
/** HRM-EXP-022/023 — staged payroll or AP payment references. */
export const hrExpensePaymentReferences = pgTable("hr_expense_payment_references", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    claimId: text("claim_id")
        .notNull()
    ...
    uniqueIndex("hr_expense_payment_refs_org_integration_uidx").on(table.organizationId, table.integrationReference),
    index("hr_expense_payment_refs_org_claim_idx").on(table.organizationId, table.claimId),
    index("hr_expense_payment_refs_org_channel_idx").on(table.organizationId, table.paymentChannel, table.active),
]);
/** HRM-EXP-028 — durable expense claim audit trail. */
export const hrExpenseAuditEvents = pgTable("hr_expense_audit_events", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    ...
    index("hr_expense_audit_events_org_occurred_idx").on(table.organizationId, table.occurredAt),
    index("hr_expense_audit_events_org_claim_idx").on(table.organizationId, table.claimId),
    index("hr_expense_audit_events_org_action_idx").on(table.organizationId, table.action),
]);
/** HRM-EXP-027 — org in-app notifications for claim workflow events. */
export const hrExpenseNotifications = pgTable("hr_expense_notifications", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    ...
    index("hr_expense_notifications_org_recipient_idx").on(table.organizationId, table.recipientAuthUserId),
    index("hr_expense_notifications_org_subject_idx").on(table.organizationId, table.subjectType, table.subjectId),
]);
