import { index, jsonb, numeric, pgEnum, pgTable, text, timestamp, uniqueIndex, } from "drizzle-orm/pg-core";
import { organizationIdColumn, timestampColumns } from "./dbx-common";
import { hrEmployees } from "./dbx-hr";
import { hrCompensationRecommendations } from "./dbx-hr-compensation-planning";
...
export const hrSbsBenchmarkVersionStatusEnum = pgEnum("hr_sbs_benchmark_version_status", ["draft", "active", "superseded", "archived"]);
export const hrSbsBenchmarkVersions = pgTable("hr_sbs_benchmark_versions", {
    ...
    uniqueIndex("hr_sbs_benchmark_versions_org_code_uidx").on(table.organizationId, table.code),
    index("hr_sbs_benchmark_versions_org_status_idx").on(table.organizationId, table.versionStatus),
]);
/** SBS-002/003/004 — market salary reference rows for a benchmark version. */
export const hrSbsBenchmarkEntries = pgTable("hr_sbs_benchmark_entries", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    ...
    uniqueIndex("hr_sbs_benchmark_entries_org_version_dims_uidx").on(table.organizationId, table.benchmarkVersionId, table.industry, table.country, table.location, table.jobFamily, table.jobLevel),
    index("hr_sbs_benchmark_entries_org_version_idx").on(table.organizationId, table.benchmarkVersionId),
]);
/** SBS-024 — currency conversion reference records. */
export const hrSbsCurrencyRateSourceEnum = pgEnum("hr_sbs_currency_rate_source", ["manual", "ecb", "survey_provider", "internal"]);
export const hrSbsCurrencyRefs = pgTable("hr_sbs_currency_refs", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    benchmarkVersionId: text("benchmark_version_id").references(() => hrSbsBenchmarkVersions.id, { onDelete: "cascade" }),
    ...
    index("hr_sbs_currency_refs_org_pair_date_idx").on(table.organizationId, table.fromCurrencyCode, table.toCurrencyCode, table.effectiveDate),
]);
...
export const hrSbsBenchmarkMappings = pgTable("hr_sbs_benchmark_mappings", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    benchmarkVersionId: text("benchmark_version_id")
        .notNull()
    ...
    index("hr_sbs_benchmark_mappings_org_version_idx").on(table.organizationId, table.benchmarkVersionId),
    index("hr_sbs_benchmark_mappings_org_status_idx").on(table.organizationId, table.mappingStatus),
    index("hr_sbs_benchmark_mappings_org_employee_idx").on(table.organizationId, table.employeeId),
]);
/** SBS-008 — approval workflow for benchmark mapping changes. */
export const hrSbsMappingApprovals = pgTable("hr_sbs_mapping_approvals", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    ...
    index("hr_sbs_mapping_approvals_org_mapping_idx").on(table.organizationId, table.mappingId),
]);
/** SBS-023 — persisted compensation analysis with benchmark version reference. */
export const hrSbsCompensationAnalyses = pgTable("hr_sbs_compensation_analyses", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    benchmarkVersionId: text("benchmark_version_id")
        .notNull()
    ...
    index("hr_sbs_comp_analyses_org_version_idx").on(table.organizationId, table.benchmarkVersionId),
    index("hr_sbs_comp_analyses_org_created_idx").on(table.organizationId, table.createdAt),
]);
/** SBS-021 — market adjustment recommendation references for CPM handoff. */
export const hrSbsCpmRecommendationRefs = pgTable("hr_sbs_cpm_recommendation_refs", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    ...
    compensationRecommendationId: text("compensation_recommendation_id").references(() => hrCompensationRecommendations.id, { onDelete: "set null" }),
    benchmarkVersionId: text("benchmark_version_id")
        .notNull()
        .references(() => hrSbsBenchmarkVersions.id, { onDelete: "restrict" }),
    ...
    index("hr_sbs_cpm_rec_refs_org_analysis_idx").on(table.organizationId, table.analysisId),
    index("hr_sbs_cpm_rec_refs_org_employee_idx").on(table.organizationId, table.employeeId),
]);
/** SBS-028 — audit trail for survey, mapping, analysis, and recommendation actions. */
export const hrSbsAuditEvents = pgTable("hr_sbs_audit_events", {
    id: text("id").primaryKey(),
    organizationId: organizationReference(),
    benchmarkVersionId: text("benchmark_version_id").references(() => hrSbsBenchmarkVersions.id, { onDelete: "set null" }),
    mappingId: text("mapping_id").references(() => hrSbsBenchmarkMappings.id, {
        onDelete: "set null",
    }),
    analysisId: text("analysis_id").references(() => hrSbsCompensationAnalyses.id, { onDelete: "set null" }),
    employeeId: text("employee_id").references(() => hrEmployees.id, {
        onDelete: "set null",
    }),
    ...
    index("hr_sbs_audit_events_org_occurred_idx").on(table.organizationId, table.occurredAt),
    index("hr_sbs_audit_events_org_action_idx").on(table.organizationId, table.action),
]);
