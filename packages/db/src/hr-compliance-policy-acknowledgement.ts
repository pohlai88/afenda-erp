...
import type { HrEmployeePolicyAcknowledgementWindow } from "./hr-compliance.types";
import { activePolicyAcknowledgementObligationKindCondition } from "./hr-compliance-policy-acknowledgement.shared";
import {
...
export async function syncHrEmployeePolicyAcknowledgementsInTx(
    ...
    db
      ...
      .where(
        and(
          ...
          activePolicyAcknowledgementObligationKindCondition,
        ),
      ),
    db
      .select({
...
export async function syncHrEmployeePolicyAcknowledgements(input: {
  organizationId: string;
}): Promise<{
  createdCount: number;
  removedCount: number;
  ...
  return runWithOrganizationContext(input.organizationId, (db) =>
    syncHrEmployeePolicyAcknowledgementsInTx(db, input),
  );
}

export async function listHrEmployeePolicyAcknowledgementsWindow(input: {
  organizationId: string;
  limit?: number;
  offset?: number;
  search?: string;
  status?: (typeof hrComplianceEmployeeRequirements.$inferSelect)["status"];
}): Promise<HrEmployeePolicyAcknowledgementWindow> {
...
export async function updateHrEmployeePolicyAcknowledgementStatusInTx(
  db: AfendaTransaction,
  input: {
    organizationId: string;
...
export async function updateHrEmployeePolicyAcknowledgementStatus(input: {
  organizationId: string;
  requirementId: string;
  status: (typeof hrComplianceEmployeeRequirements.$inferSelect)["status"];
  reviewNotes?: string | null;
  ...
  return runWithOrganizationContext(input.organizationId, (db) =>
    updateHrEmployeePolicyAcknowledgementStatusInTx(db, input),
  );
}
