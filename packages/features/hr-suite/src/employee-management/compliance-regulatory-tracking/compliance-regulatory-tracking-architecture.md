...
**Route:** `/hr/compliance` · **Module:** `@afenda/feature-hr-suite` · **Capabilities:** `hr.compliance.read`, `hr.compliance.write`, `hr.compliance.sensitive.read`
...
| DB commands | `@afenda/db` → `hr-compliance.ts` and `hr-compliance-*.ts` modules |
| Export doors | `server.ts` (I/O), `client.ts` (components), `metadata.ts` (keys, copy, search params) |


### Workbench composition

1. **Pattern B** — overview KPI stat groups (`hr.workforce.compliance.overview.stats`) + read-only breakdown list (`hr.workforce.compliance.overview-breakdown.list`) — HRM-CMP-022
2. **Fifteen Pattern C lists** — registry `HR_COMPLIANCE_LIST_SURFACE_KEYS` in `surface/hr.workforce.compliance-surface-metadata.shared.ts`
3. **Reports export panel** — CSV via `exportHrComplianceReportAction` — HRM-CMP-023 (before audit trail)
4. Optional obligation upsert form when `hr.compliance.write` is granted

Pattern C registry order: alerts → review queue → obligations → filings → regulatory calendar → policy acknowledgements → labor law → statutory → safety training → workplace safety → work eligibility → work auth → exceptions → evidence links → audit trail.

Overview surfaces and the reports panel are **not** in `HR_COMPLIANCE_LIST_SURFACE_KEYS`.

### HRM-CMP shipment matrix


| Code | Status |
| ---- | ------ |
| HRM-CMP-001 … HRM-CMP-020 | **Shipped** — see per-code as-built sections below |
| HRM-CMP-021 | **Shipped** (central inbox + approve/reject; multi-step routing **future**) |
| HRM-CMP-022 | **Shipped** |
| HRM-CMP-023 | **Shipped** |
| HRM-CMP-024 | **Shipped** |
| HRM-CMP-025 | **Shipped** |
...
Per-code as-built detail follows in the sections below.
