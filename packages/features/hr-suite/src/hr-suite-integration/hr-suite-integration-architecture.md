...
| Neutral contracts | `contracts/hr-suite-module.contract.ts`, `contracts/hr-suite-pagination.contract.ts`, `contracts/hr-suite-permission.contract.ts` |
| Metadata navigation | `navigation/hr-suite-nav.contract.ts` |
| Server policy | `policies/hr-suite-access.policy.server.ts` |
| Metadata surface helpers | `surface/hr-suite-list-surface.shared.ts` |
