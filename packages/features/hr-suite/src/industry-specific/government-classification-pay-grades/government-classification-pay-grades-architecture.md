...
Status: shipped contract for
`industry-specific/government-classification-pay-grades` with domain key
`hr.industry.gpg` and route `/hr/government-classification-pay-grades`.

| Area               | Implementation                                                                                                                                                                                                                                                     |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Domain model       | GPG exports cover government classifications, pay grades and bands, salary table versions, locality and allowance references, step eligibility, classification assignments, grade movement, retention, reviews, integration exposures, reports, and audit history. |
| Governed UI        | List metadata exposes Pattern C server-window surfaces through `HR_INDUSTRY_GPG_LIST_SURFACE_KEYS`, read-only surface metadata, `buildHrIndustryGpgListSurface`, and `buildHrIndustryGpgPageModel`.                                                                |
| Access             | Read permission constants gate list surfaces, while page model inputs carry write, approval, audit, restricted-read, integration exposure, and employee visibility gates.                                                                                          |
| Search and filters | `parseHrIndustryGpgSearchParams` and `toHrIndustryGpgPageModelInput` normalize per-surface searches, status filtering, report grouping, organization context, and visibility scope.                                                                                |
| Integrations       | Integration exposure surfaces publish approved grade, step, salary table, locality adjustment, allowance, and grade movement references for Payroll Processing and Employee Lifecycle Management.                                                                  |
| Coverage           | `HR_INDUSTRY_GPG_REQUIREMENT_COVERAGE` marks `HRM-GPG-001..031` as shipped, and `HR_INDUSTRY_GPG_ACCEPTANCE_CRITERIA_COVERAGE` maps the 26 enterprise acceptance criteria with package-level tests.                                                                |
