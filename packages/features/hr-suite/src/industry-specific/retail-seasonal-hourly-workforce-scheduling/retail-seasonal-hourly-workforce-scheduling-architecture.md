...
`src/industry-specific/retail-seasonal-hourly-workforce-scheduling` with domain
key `hr.industry.rws`, route `/hr/retail-seasonal-hourly-workforce-scheduling`,
and capability prefix `hr.rws.*`.

The implementation covers `HRM-RWS-001` through `HRM-RWS-034` with typed
schemas, tenant-scoped seeded stores, governed list surfaces, overview KPI
metadata, server actions, permission gates, audit events, app route adapter,
navigation, auth capability catalog, kernel route mapping, and permission seed
metadata.

Cost and budget fields are gated by `hr.rws.labor-cost.read`; restricted
availability, blocked-date, minor/student/restricted-worker, compliance finding,
and override reason details are gated by `hr.rws.restricted.read`; integration
exports are gated by `hr.rws.integration.expose`; audit history is gated by
`hr.rws.audit.read`. Payroll handoff is represented through attendance outcome
references instead of direct payroll calculation.

Package-level tests validate coverage, search parameters, metadata-driven list
configuration, cost/restricted redaction, integration references, and shipped
vertical naming.
