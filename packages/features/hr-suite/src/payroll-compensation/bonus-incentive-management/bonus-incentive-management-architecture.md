...
## HRM-BON-013 … HRM-BON-018 As-built (governed UI)

**Route:** `/hr/bonus` · **Capabilities:** `hr.bonus.read`, `hr.bonus.write`

Six embedded **Pattern C** lists via `HrBonusWorkbenchSection` (`@container`, `GovernedPatternCListSection`, `erp-operational-table`, `requiresErpPermission: hr.bonus.read`):

| Code | Surface key | Table |
| ---- | ----------- | ----- |
| HRM-BON-013 | `hr.payroll.bonus.guaranteed-rules.list` | `hr_bonus_guaranteed_rules` |
| HRM-BON-014 | `hr.payroll.bonus.multipliers.list` | `hr_bonus_performance_multipliers` |
| HRM-BON-015 | `hr.payroll.bonus.prorations.list` | `hr_bonus_prorations` |
| HRM-BON-016 | `hr.payroll.bonus.manual-adjustments.list` | `hr_bonus_manual_adjustments` |
| HRM-BON-017 | `hr.payroll.bonus.discretionary.list` | `hr_bonus_discretionary_recommendations` |
| HRM-BON-018 | `hr.payroll.bonus.recoveries.list` | `hr_bonus_recoveries` |

**ERP:** `apps/erp/src/lib/hr-sections/bonus.server.tsx` · **Feature:** `packages/features/hr-suite/src/payroll-compensation/bonus-incentive-management/`

**Gallery:** No new governed renderer ids (operational tables only); metadata-renderer-gallery unchanged.

---

## Shipment (HRM-BON-025..030)

**Capabilities:** `hr.bonus.read`, `hr.bonus.write`, `hr.bonus.sensitive.read`, `hr.bonus.finance.read`, `hr.bonus.audit.read`

| Code | Status | As-built |
| ---- | ------ | -------- |
| HRM-BON-025 | **Shipped** | `lockHrBonusPayoutAfterFinalApprovalInTx`, `assertHrBonusPayoutEditableInTx`; `hr_bonus_payouts.locked_at` + `payout_status = locked` |
| HRM-BON-026 | **Shipped** | `_integration/payroll-bonus-payouts.server.ts` + `listHrBonusPayrollPayoutRefs` / `markHrBonusPayrollPayoutRefsSyncedInTx` (reference-only; Payroll Processing consumes) |
| HRM-BON-027 | **Shipped** | `updateHrBonusPayoutAccountingAllocationInTx`; allocation columns on `hr_bonus_payouts` |
| HRM-BON-028 | **Shipped** | `buildHrBonusReportCsv` — kinds `bonus`, `commission`, `incentive`, `payout_variance`, `eligibility` |
| HRM-BON-029 | **Shipped** | `hr.payroll.bonus-access.policy.server.ts`; sensitive amount masking via `hr.bonus.sensitive.read` |
| HRM-BON-030 | **Shipped** | `hr_bonus_audit_events`; `hrPayrollBonusAuditActions`; `appendHrBonusAuditEventInTx` on lock, payroll sync, allocation |

| AC | Covered by |
| -- | ---------- |
| 19 | BON-025 lock after final approval |
| 20 | BON-026 payroll payout references |
| 21 | BON-027 accounting allocation |
| 23 | BON-028 CSV reports |
| 24 | BON-029 capability gates |
| 25 | BON-030 domain audit events |

**Tests:** `bonusincentive-lock.test.ts`, `bonusincentive-access.policy.test.ts`, `bonusincentive-audit.test.ts`, `bonusincentive-acceptance-coverage.test.ts`.

### Payroll Processing integration boundary

| Layer | Path | Role |
| ----- | ---- | ---- |
| DB export | `packages/db/src/hr-bonus-payroll.ts` | `listHrBonusPayrollPayoutRefs` — locked approved payouts with earnings refs |
| Feature bridge | `payroll-compensation/_integration/payroll-bonus-payouts.server.ts` | `listApprovedBonusPayrollPayoutRefs`, `acknowledgeBonusPayrollPayoutSync` |
| Payroll Processing | `payroll-compensation/payroll-processing/` | **Consumer** — reads refs; owns calculation and run finalization |

Bonus & Incentive Management **does not** calculate net pay, apply statutory rules, or finalize payroll runs.

## Shipment (HRM-BON-019..024)

| Code | Status | As-built |
| ---- | ------ | -------- |
| HRM-BON-019 | **Shipped** | `validateHrBonusEligibilityBeforePayoutInTx` + `prepareHrBonusPayoutInTx` before calculation |
...
| AC | Covered by |
| -- | ---------- |
| 15 | BON-020 validation flags |
| 16 | BON-021 approval workflow |
| 17 | BON-023 approver decisions |
| 18 | BON-024 required reasons |

Final approve calls `lockHrBonusPayoutAfterFinalApprovalInTx` (BON-025). BON-001..006 plan/cycle/payout foundation lives in `packages/db/src/schema/hr-bonus-incentive.ts` and `hr-bonus-*.ts`.

**Tests:** `bonus-workflow-validation.test.ts`, `bonus-workflow-approval.test.ts`, `bonusincentive-acceptance-coverage.test.ts`.
