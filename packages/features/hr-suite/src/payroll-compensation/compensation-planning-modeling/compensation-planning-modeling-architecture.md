...
## Payroll Processing integration boundary (HRM-CPM-027)

| Layer | Path | Role |
| ----- | ---- | ---- |
| DB | `packages/db/src/hr-compensation-planning.ts` | `finalizeHrCompensationApprovalInTx`, `listHrCompensationPayrollRefs`, `markHrCompensationPayrollRefsSyncedInTx` |
| Feature bridge | `payroll-compensation/_integration/payroll-compensation-changes.server.ts` | `listApprovedCompensationPayrollRefs`, `acknowledgeCompensationPayrollSync` |
| Payroll Processing | `payroll-compensation/payroll-processing/` | **Consumer** — reads pending refs; owns calculation and run finalization |

Compensation Planning & Modeling **does not** calculate net pay or finalize payroll runs. Finalize creates `hr_compensation_salary_changes`, links `hr_employee_record_events` via `employeeHistoryEventId`, and queues `hr_compensation_payroll_refs` (`sync_status = pending`). Re-finalize is idempotent when artifacts already exist.

**Tests:** `tests/unit/cpm-payroll-integration.test.ts`, `cpm-audit.test.ts`, `cpm-acceptance-coverage.test.ts`.
