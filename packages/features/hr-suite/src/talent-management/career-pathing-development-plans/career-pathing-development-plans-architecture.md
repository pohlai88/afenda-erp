...
- **Slice 4 (shipped):** Learning actions (training course ref), stretch assignments, `@afenda/feature-hrm-talent-management/server` learning refs export.
- **Slice 5 (shipped):** Mentor assignment, career discussions, manager review on plans.
- **Slice 6 (shipped):** Readiness snapshots, overview KPIs, overdue milestone counts, succession readiness export helper, org notifications on overdue milestones / completed goals / career discussions, readiness CSV export (`exportCareerPathReadinessCsvAction`).
...
| `career-pathing-overview-section.tsx` | KPI counts + near-ready tally |
...
| `surfaceKey` | Section |
| --- | --- |
| `hrm:career-pathing:overview-kpi` | Overview KPI strip |
...
- **Training:** deep link to `/apps/hrm/training`; `listDevelopmentLearningRefsForEmployee` on `@afenda/feature-hrm-talent-management/server`.
- **Succession:** `listReadinessRefsForSuccession` on `@afenda/feature-hrm-talent-management/server` (module not yet built).
...
| **Development progress overview** | Completed goals, overdue milestones, active actions, readiness progress                         |
