...
export async function getLmsComplianceCompletionSnapshot(input: {
  ...
  try {
    ...
    const assignments = listHrLmsAssignmentsFromStore(input.organizationId).filter(
      (row) => row.isComplianceMandatory,
    );
    const progress = listHrLmsProgressFromStore(input.organizationId, input.employeeIds);
...
export async function getLmsTrainingDevelopmentRefs(input: {
  ...
  try {
    ...
    const courses = listHrLmsCoursesFromStore(input.organizationId);
    const progress = listHrLmsProgressFromStore(input.organizationId, input.employeeIds).filter(
