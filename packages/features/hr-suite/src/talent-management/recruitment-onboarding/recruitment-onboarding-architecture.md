...
Recruitment & Onboarding now ships as an HR Suite vertical slice under
`talent-management/recruitment-onboarding` with:

- feature-local constants, Zod schemas, route/permission contracts, audit action
  taxonomy, package-local store, reports, integration references, and access
  policies;
- governed Pattern C list surfaces for requisitions, postings, applications,
  interviews, offers, onboarding tasks, readiness, reports, and audit trail;
- execution-kernel read/write/approval/interview/offer/onboarding/finance/IT/
  sensitive/audit/conversion capability gates;
- `/hr/recruitment-onboarding` thin App Router adapter; and
- Vitest coverage for HRM-RON-001 through HRM-RON-041 and the 33 enterprise
  acceptance criteria.

Durable recruitment persistence is intentionally deferred because `@afenda/db`
does not yet own requisition, candidate, application, screening, interview,
scorecard, assessment, offer, or recruitment-audit tables. Existing DB-backed
onboarding case primitives remain downstream references until candidate
conversion and employee onboarding status promotion are aligned through the
