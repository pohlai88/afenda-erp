...
# Coexistence with Learning Management System (`/apps/hrm/lms`)

| Workflow | Owner | Notes |
| --- | --- | --- |
| Classroom delivery, attendance, waitlist, training cost | TRN (this module) | Canonical classroom engine |
| Online courses, learning paths, e-learning progress | LMS | TRN reads blended completion via `training-lms-bridge.server.ts` when `hrm_lms_course.trainingCourseId` is set |
