...
| LAM ID | Status | Implementation |
| ------ | ------ | -------------- |
| HRM-LAM-001 | Done | `hr_attendance_days` + `upsertHrAttendanceDay` / `listHrAttendanceDaysWindow` |
| HRM-LAM-002 | Done | `hr_attendance_day_status` enum (present … missing_punch) |
| HRM-LAM-003 | Done | `hr_leave_type_configs` by `policyGroupCode` |
| HRM-LAM-004 | Done | `hr_leave_entitlement_rules` scoped by entity, country, location, type, grade, tenure |
| HRM-LAM-005 | Done | `calculateHrLeaveEntitlementForEmployee` |
| HRM-LAM-006 | Done | `hr_leave_balances` + remaining balance computation |
