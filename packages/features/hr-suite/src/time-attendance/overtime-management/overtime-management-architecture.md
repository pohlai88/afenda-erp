...
| **3 — UI** | `…/overtime-management/components/*.tsx` + `@afenda/feature-hrm-time-attendance/client` | Forms, Pattern C inboxes, policy dialog; client islands import the owning package client door only |

**Public doors:** `@afenda/feature-hrm-time-attendance` (`OvertimePage`, `resolveOtmSurfaceAccess`) · `@afenda/feature-hrm-time-attendance/client` (actions + client forms) · `@afenda/feature-hrm-time-attendance/server` (`runOtmApprovalOverdueTick` for cron).
