...
# Slice 2d — As-built (Afenda ERP)

Parent: [ARCH-010 HR feature package](../../../../../../docs/architecture/010-hr-feature-package-architecture.md) · roadmap [004-hrm-migration](../../../../../../docs/roadmap/004-hrm-migration.md) slice **2d**.

## Package layout

| Layer | Location |
| ----- | -------- |
| Commands / list windows | `@afenda/db` → `packages/db/src/hr-compliance.ts` |
| Schema | `packages/db/src/schema/hr.ts` → `hr_compliance_obligations`, `hr_compliance_exceptions` |
| Feature vertical | `packages/features/hr/src/workforce/compliance/` |
| App route | `apps/erp/src/lib/hr-sections/compliance.server.tsx` → `/hr/compliance` |
| Permissions | `hr.compliance.read`, `hr.compliance.write` |

## Migrated capabilities (legacy `compliance-regulatory-tracking`)

| Capability | Afenda surface |
| ---------- | -------------- |
| Obligation register (upsert by code, archive) | `upsertHrComplianceObligation`, `archiveHrComplianceObligation` + upsert/archive forms |
| Open exceptions queue (bounded list window) | `listHrComplianceExceptionsWindow` + governed Pattern C list |
| Create exception (optional corrective action at create → `in_progress`) | `createHrComplianceException` |
| Assign corrective action | `assignHrComplianceCorrectiveAction` |
| Corrective action progress (appended dated notes in description) | `updateHrComplianceCorrectiveActionProgress` |
| Resolve exception | `resolveHrComplianceException` |
| Waive exception (reason + approval reference) | `waiveHrComplianceException` |
| Server actions + execution audit | `hr-compliance.actions.server.ts` → `writeExecutionAuditEvent` (`hr.compliance.*` actions) |
| Tenancy | Execution context `organizationId` — no `orgSlug` in forms |

Revalidation path: `/hr/compliance` (not legacy `/hrm/compliance`).

## Legacy gap analysis (foundation scope)

| Legacy area | Status in Afenda |
| ----------- | ---------------- |
| Obligation upsert/archive actions | **Migrated** (narrower obligation fields; see deferred) |
| Exception create / assign / progress / resolve / waive | **Migrated** |
| Obligations + exceptions governed lists | **Migrated** |
| Employee picker on exception create | **Migrated** (`loadHrLifecycleFormOptions`) |
| Operational health / overview dashboards | **Deferred** |
| Employee compliance status panels | **Deferred** |
| Evidence register + statutory pack upsert/submission | **Deferred** (no `hr_compliance_evidence` table) |
| Filings register, regulatory calendar UI | **Deferred** |
| Compliance reports / CSV export actions | **Deferred** |
| `correctiveActionOwnerUserId` on exceptions | **Deferred** (no owner column; assign sets description + due date only) |
| `sourceReferenceId` + auto-generated exceptions (document expiry watch) | **Deferred** |
| Evidence document link on resolve/progress | **Deferred** |
| Cron: aging watch, control watch, statutory retry | **Deferred** |
| LMS bridge, bureau reliability queries | **Deferred** |
| Statutory submission / acknowledgement writers | **Deferred** |

## Explicitly deferred (do not implement in slice 2d without schema ADR)

- **Cron / watches:** `compliance-aging-watch`, `compliance-control-watch`, document-expiry auto-exception upsert
- **LMS bridge** and training-compliance sync
- **Bureau reliability** read models and statutory retry jobs
- **Statutory submission writers** (`statutory-submission.actions`, `statutory-acknowledgement.actions`, evidence regeneration snapshots)
- **Evidence / filings tables** and document linking on exceptions
- **Extended obligation dimensions** (country, legal entity, work location, employment type, worker category, policy version, alert lead days) unless product requests schema extension via `pnpm db:generate`
...
pnpm --filter @afenda/feature-hr test -- compliance
```
