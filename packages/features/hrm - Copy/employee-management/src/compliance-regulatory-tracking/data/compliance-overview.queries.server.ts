...
import {
  deriveComplianceOverviewOverallStatus,
  type ComplianceOverviewFilterInput,
  type ComplianceOverviewRow,
} from "./compliance-overview.shared"
import {
  groupLmsMandatoryComplianceRowsByEmployee,
  listLmsMandatoryComplianceRowsForOrg,
...
function matchesFilter(
  row: ComplianceOverviewRow,
  filter: ComplianceOverviewFilterInput
): boolean {
  return (
...
export async function listComplianceOverviewRowsForOrg(
  ...
  const [
    ...
    listLmsMandatoryComplianceRowsForOrg({ organizationId }),
  ])
  ...
  const lmsTrainingByEmployee =
    groupLmsMandatoryComplianceRowsByEmployee(lmsMandatoryRows)

  const rows: ComplianceOverviewRow[] = employees.map((employee) => {
    ...
    const employeeTrainingStatuses = [
      ...(trainingByEmployee.get(employee.id) ?? []).map((row) =>
        deriveTrainingComplianceStatus({
          completedAt:
      ...
      ...(lmsTrainingByEmployee.get(employee.id) ?? []).map((row) =>
    ...
    const overallStatus = deriveComplianceOverviewOverallStatus({
