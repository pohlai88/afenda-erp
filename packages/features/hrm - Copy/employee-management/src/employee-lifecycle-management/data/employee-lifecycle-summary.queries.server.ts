...
import { summarizeOnboardingLearningCompletion } from "./boarding-lms-bridge.server"
...
export type EmployeeLifecycleSnapshot = {
  ...
  /** HRM-LMS-022 — onboarding-path LMS enrollments for this employee. */
  readonly onboardingLearningTotal: number
  readonly onboardingLearningCompleted: number
  readonly onboardingLearningInProgress: number
  readonly onboardingLearningBlocked: number
...
export async function getEmployeeLifecycleSnapshot(
  ...
  const [boarding, offboarding, pendingTransitions, onboardingLearning] =
    await Promise.all([
  ...
  return {
    ...
    onboardingLearningTotal: onboardingLearning.total,
    onboardingLearningCompleted: onboardingLearning.completed,
    onboardingLearningInProgress: onboardingLearning.inProgress,
    onboardingLearningBlocked: onboardingLearning.blocked,
