...
import { hrmBenefitLifeEvent } from "@afenda/platform/db/schema"
...
import { getEmployeeForOrganization } from "@afenda/feature-hrm-employee-management/server"
import { requireHrmAdmin } from "@afenda/feature-hrm-core/governance"
import {
...
export async function recordLifeEventAction(
  ...
  after(() =>
    writeIamAuditEventFromNextHeaders({
      action: HRM_BENEFIT_AUDIT.life_event.record,
      organizationId,
      actorUserId: userId,
...
export async function verifyLifeEventAction(
  ...
  after(() =>
    writeIamAuditEventFromNextHeaders({
      action: HRM_BENEFIT_AUDIT.life_event.verify,
      organizationId,
      actorUserId: userId,
      actorSessionId: sessionId,
      resourceType: "hrm_benefit_life_event",
