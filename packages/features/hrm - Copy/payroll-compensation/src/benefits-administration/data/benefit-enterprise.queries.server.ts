...
import { resolveEmployeeLegalEntityCode } from "./benefit-legal-entity.server"
...
export async function evaluateBenefitEligibilityForEmployee(
  ...
  const [plan] = await db
    .select({
      ...
      scopeCountryCodes: hrmBenefit.scopeCountryCodes,
      scopeLegalEntityCodes: hrmBenefit.scopeLegalEntityCodes,
  ...
  const [dependentCount, legalEntityCode] = await Promise.all([
    getDependentCount(options.organizationId, options.employeeId),
    resolveEmployeeLegalEntityCode(
    ...
    await Promise.all([
      db
        .select({
          ...
          benefitCategory: hrmBenefit.benefitCategory,
          ...
          providerId: hrmBenefit.providerId,
          scopeCountryCodes: hrmBenefit.scopeCountryCodes,
          scopeLegalEntityCodes: hrmBenefit.scopeLegalEntityCodes,
          eligibilityRules: hrmBenefit.eligibilityRules,
