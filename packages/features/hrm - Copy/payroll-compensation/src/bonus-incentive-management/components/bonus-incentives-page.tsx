...
import { Card, CardHeader, CardTitle } from "@afenda/ui/card"
  ...
  return (
    <nav aria-label="Bonus incentive sections" className="flex flex-wrap gap-2">
      {BONUS_INCENTIVE_TABS.map((tab) => (
        <Link
          key={tab}
          href={tabHref(orgSlug, tab) as Route}
...
export async function BonusIncentivesPage({
  ...
  const [
    ...
    getTranslations("Erp.Hrm.bonusIncentives"),
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      {activeTab === "clawbacks" ? (
        <BonusClawbacksSection orgSlug={orgSlug} rows={clawbacks} />
      ) : null}
    </div>
  )
}
