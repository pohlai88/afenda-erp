...
export function CreatePayrollPeriodForm({ canCreate }: { canCreate: boolean }) {
  const t = useTranslations("Erp.Hrm.payroll")
...
export function PayrollClosePassport({
  ...
  return (
    <div className="flex flex-col gap-5 rounded-md border border-border bg-muted/20 p-4">
...
export function PayrollPeriodDetailCard({
  ...
  return (
    <Card id={`payroll-period-${period.id}`} size="sm">
      ...
      <CardContent className="flex flex-col gap-6">
        ...
        {isPreparing && !isLocked && (
          <div className="flex flex-col gap-4 rounded-md border border-border bg-muted/30 px-4 py-4">
            <p className="text-sm text-muted-foreground">
              {t("lockWorkflowHint")}
            </p>
            <div className="flex flex-col gap-4">
