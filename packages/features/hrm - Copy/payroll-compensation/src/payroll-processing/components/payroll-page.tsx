import "server-only"
import { HrmShellAccessDeniedFromNav } from "@afenda/feature-hrm-core/registry"
...
import { PayrollPeriodsListSection } from "./payroll-periods-list-section"
...
export async function PayrollPage() {
  const [capabilities, session, t] = await Promise.all([
    ...
    requireOrgSession(),
    getTranslations("Erp.Hrm.payroll"),
  ])
  ...
  const overviewPeriods = periodViews.map((view) => view.period)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {overviewPeriods.length > 0 ? (
        <PayrollPeriodsListSection periods={overviewPeriods} />
      ) : null}
