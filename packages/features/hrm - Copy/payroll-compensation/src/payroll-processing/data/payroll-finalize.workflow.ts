...
import {
  ...
  insertPlannerSignal,
} from "../../_integration/orbit.server"
...
import {
  PAYROLL_FINALIZE_RUN_FAILED_AUDIT_ACTION,
  type PayrollFinalizePayload,
} from "./payroll-finalize-payload.shared"
import { resolveRulePack } from "../../multi-country-payroll/data/payroll-rule-pack.server"

export type { PayrollFinalizePayload } from "./payroll-finalize-payload.shared"
...
async function payrollPrepareFailedStep(
  ...
  await writeIamAuditEvent({
    action: PAYROLL_FINALIZE_RUN_FAILED_AUDIT_ACTION,
    actorUserId: payload.actorUserId,
    actorSessionId: payload.actorSessionId,
    organizationId: payload.organizationId,
    resourceType: "hrm_payroll_period",
