...
import { CareerPathStageCreateForm } from "./career-pathing-forms.client"
import { CareerPathFrameworkStagesTrailingCell } from "./career-pathing-framework-stages-trailing-cell.client"
...
export async function CareerPathingFrameworkStagesSection({
  ...
  try {
    stages = await listCareerPathStagesForFramework(
      organizationId,
      framework.id
    )
  } catch (err) {
    logUnexpectedServerError(
      "career-pathing-framework-stages: query failed",
      err,
      {
        organizationId,
  ...
  return (
    <div className="flex flex-col gap-3 border-t pt-4">
      ...
      <GovernedPatternCListSection
        ...
        listConfiguration={buildCareerPathStagesListSurfaceConfiguration(
          stages,
          {
            title: t("frameworkStagesTitle"),
            description: t("frameworkStagesDescription"),
        ...
        trailingColumn={{
          header: "",
          Cell: CareerPathFrameworkStagesTrailingCell,
          context: {
            organizationId,
            orgSlug,
            deleteLabel: t("deleteStage"),
          },
        }}
      />
    </div>
