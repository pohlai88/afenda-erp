...
import { CareerPathPlanGoalTrailingCell } from "./career-pathing-list-trailing-cells.client"
...
export async function CareerPathingPrimaryPlanSection({
  ...
  let planLearningActions: Awaited<
    ReturnType<typeof listLearningActionsForPlan>
  > = []
  ...
  try {
    ;[planGoals, planLearningActions, planStretchAssignments] =
      await Promise.all([
    ...
    logUnexpectedServerError(
      "career-pathing-primary-plan: detail failed",
      err,
      {
        organizationId,
        planId: plan.id,
    ...
    <div className="flex flex-col gap-3 border-t pt-4">
      ...
      {planDetailLoadFailed ? (
        ...
        <GovernedPatternCListSection
          ...
          trailingColumn={{
            header: "",
            Cell: CareerPathPlanGoalTrailingCell,
            context: {
              organizationId,
              orgSlug,
              markInProgressLabel: t("markInProgress"),
            },
          }}
        />
      )}
