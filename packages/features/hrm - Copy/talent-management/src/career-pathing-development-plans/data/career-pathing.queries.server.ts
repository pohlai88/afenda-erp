...
export async function listTargetRolesForOrg(
  ...
  const rows = await db
    ...
    .innerJoin(
      hrmEmployee,
      eq(hrmEmployee.id, hrmEmployeeTargetRole.employeeId)
    )
    .leftJoin(
      hrmCareerPathFramework,
...
export async function listCareerDiscussionsForOrg(organizationId: string) {
  return db
    .select({
      ...
      employeeId: hrmCareerDiscussion.employeeId,
      employeeName: hrmEmployee.legalName,
  ...
  const [row] = await db
    ...
    .where(
      and(
        ...
        inArray(hrmDevelopmentMilestone.status, ["not_started", "in_progress"]),
