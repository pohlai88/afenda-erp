...
**Learning Management System (LMS) is the HRM function that delivers, manages, tracks, and reports employee learning through online courses, learning paths, certifications, compliance training, assessments, progress overview surfaces, and completion records.**
...
| **Learning overview**          | Employee progress, team progress, overdue training, completion percentage                           |
| **Manager overview**           | Team learning status, mandatory training completion, certification risk                             |
| **HR overview**                | Organization-wide completion, compliance training status, skills development status                 |
...
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Learning Management System (LMS)** | Delivers and tracks online courses, learning paths, certifications, assessments, and compliance training with employee, manager, and HR progress overview surfaces, reminders, completion records, reporting, integration references, and audit history. |
...
| **HRM-LMS-024** | System shall provide employee learning overview.                                                                                                                                                               |
| **HRM-LMS-025** | System shall provide manager team-learning overview.                                                                                                                                                           |
| **HRM-LMS-026** | System shall provide HR learning administration overview.                                                                                                                                                      |
...
|  21 | Employee can view own learning progress overview.                                                                                                               |
|  22 | Manager can view team learning progress where authorized.                                                                                                        |
|  23 | HR can view organization-wide learning completion and overdue training overview surfaces.                                                                               |
...
# Coexistence with Training & Development (`/apps/hrm/training`)

Both **LMS** (`/apps/hrm/lms`) and **Training & Development** (`/apps/hrm/training`) remain separate nav entries.

| Workflow | Owner | Notes |
| --- | --- | --- |
| Classroom sessions, waitlist, attendance, roster | TRN | LMS does not duplicate |
| Employee skill profiles, competency gaps, dev plans | TRN | LMS exposes completion refs only (HRM-LMS-023) |
| Online/e-learning catalog, paths, SCORM/xAPI refs | LMS | Optional `trainingCourseId` FK for blended rows |
| Mandatory compliance posture in Compliance module | Compliance | Reads TRN assignments **and** LMS mandatory rows via `compliance-lms-bridge.server.ts` (HRM-LMS-021) |
| Onboarding learning completion | Lifecycle | Reads LMS onboarding paths via `boarding-lms-bridge.server.ts` (HRM-LMS-022); completes boarding tasks on LMS course completion (`metadata.lmsCourseCode`) |
| Blended completion refs on TRN analytics | TRN | Reads LMS via `training-lms-bridge.server.ts` (HRM-LMS-023) |

**Integration doors (server-only):** `@afenda/feature-hrm-talent-management/server` exports `getLmsComplianceCompletionSnapshot`, `getLmsOnboardingCompletionSnapshot`, and `getLmsTrainingDevelopmentRefs`. Consumer bridges live in the owning module — never duplicate LMS mutations inside TRN or Compliance.
