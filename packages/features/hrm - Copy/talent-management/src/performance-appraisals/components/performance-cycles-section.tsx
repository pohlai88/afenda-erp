...
import { PerformanceCyclesTrailingCell } from "./performance-list-trailing-cells.client"

function pipelineLabel(
  pipeline: HrmReviewPipeline,
  t: Awaited<ReturnType<typeof getTranslations<"Erp.Hrm.performance">>>
): string {
...
export async function PerformanceCyclesSection({
  ...
  const [t, format] = await Promise.all([
    getTranslations("Erp.Hrm.performance"),
    getFormatter(),
  ])
  ...
  return (
    <GovernedPatternCListSection
      ...
      trailingColumn={{
        header: t("colActions"),
        Cell: PerformanceCyclesTrailingCell,
        context: {
          orgSlug,
          canUpdate,
          cycles,
