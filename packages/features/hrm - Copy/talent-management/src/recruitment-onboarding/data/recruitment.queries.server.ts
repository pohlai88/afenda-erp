...
export async function listApplicationsForOrg(
  ...
  const rows = await db
    .select({
      ...
      candidateResumeUrl: hrmCandidate.resumeUrl,
      ...
      createdAt: hrmApplication.createdAt,
      updatedAt: hrmApplication.updatedAt,
      createdByUserId: hrmApplication.createdByUserId,
      updatedByUserId: hrmApplication.updatedByUserId,
    })
    .from(hrmApplication)
    .innerJoin(
      hrmCandidate,
