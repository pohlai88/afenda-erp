...
import type { CreateOtmEligibilityRuleFormState } from "@afenda/feature-hrm-core/shared"
import { createOtmEligibilityRuleFormSchema } from "../schemas/otm.schema"
import { createOtmEligibilityRule } from "../data/otm-eligibility.server"

export async function createOtmEligibilityRuleAction(
  _prev: CreateOtmEligibilityRuleFormState | undefined,
  formData: FormData
): Promise<CreateOtmEligibilityRuleFormState> {
  ...
  const parsed = createOtmEligibilityRuleFormSchema.safeParse({
  ...
  if (!parsed.success) {
    return hrmActionFailure({
      ...
      overtimeTypeId: parsed.error.flatten().fieldErrors.overtimeTypeId?.[0],
    })
  }

  const result = await createOtmEligibilityRule({
    organizationId,
    userId,
    sessionId,
