...
import { hrmLeaveBalance, hrmLeaveType, iamAuditEvent } from "@afenda/platform/db/schema"
...
export async function applyOtmCompensatoryLeaveCredit(input: {
  ...
  if (!Number.isFinite(entitlementYear)) {
    return {
      status: "failed",
      reason: "Invalid work date for entitlement year.",
    }
  }
  ...
  const now = new Date()
  const adjustedDays = Number(existing.adjustedDays) + leaveDays
