...
import { hrmApproval, hrmOvertimeRequest } from "@afenda/platform/db/schema"

import { hrmActionFailure } from "@afenda/feature-hrm-core/governance"
import type { OtmRequestMutationFormState } from "@afenda/feature-hrm-core/shared"
import {
  HRM_OTM_AUDIT,
...
export async function submitOtmRequest(
  ...
  if (activeTypeCount > 0) {
    ...
    const overtimeType = await getOtmTypeForOrg(organizationId, overtimeTypeId)
    if (!overtimeType) {
    ...
    dayCategory = parsedCategory.success ? parsedCategory.data : "normal_day"
  ...
  if (
    ...
    return hrmActionFailure({
      workDate: "Work date is outside the configured overtime claim deadline.",
    })
  }
