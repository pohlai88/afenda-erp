...
import { hrmTimeClockPunchException } from "@afenda/platform/db/schema"
...
import { requireTimeClockExceptionDecisionPermission } from "./tci-correction-access.server"
import {
...
export async function decideTimeClockPunchException(
  ...
  const access = await requireTimeClockExceptionDecisionPermission()
  if (!access.ok) {
  ...
  if (
    ...
    return hrmActionFailure({
      form: "Session does not match correction authority.",
    })
  }
  ...
  if (
    ...
    return hrmActionFailure({
      form: "Unsupported punch event type on exception.",
    })
  }
