...
import { parsePageHeaderData } from "@afenda/governed-surface"
import type { PageHeader } from "@afenda/governed-surface"

import { organizationHrmPath } from "../../_core/shared"

type HrmModulePageHeaderKeys = {
...
type HrmModulePageHeaderNavOverride = {
  href: string
  label: string
}
 ...
 * back to the HRM overview unless `navOverride` is supplied (e.g. landing
 * page returns to the ERP home).
 */
export async function buildGovernedHrmModulePageHeader(
  orgSlug: string,
  bodyNamespace: string,
  keys: HrmModulePageHeaderKeys,
  navOverride?: HrmModulePageHeaderNavOverride
): Promise<PageHeader> {
  const [tBody, tShell] = await Promise.all([
    // Call sites use `Erp.Hrm.*` namespaces from the message catalog; the
  ...
  if (!parsed.success) {
    throw new Error(
      `buildGovernedHrmModulePageHeader: invalid header for ${bodyNamespace}`
    )
  }
  return parsed.data
}
