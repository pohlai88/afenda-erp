...
export const HRM_COMPLIANCE_EXECUTION_AUDIT_ACTIONS = {
 ...
 * regulator views, log drains, and `AGENTS.md` in the same commit.
 */
...
function buildAuditActionToKindMap(): Record<string, ComplianceTimelineKind> {
  const map: Record<string, ComplianceTimelineKind> = {
    ...
    [HRM_COMPLIANCE_EXECUTION_AUDIT_ACTIONS.STATUTORY_SUBMISSION_DELIVERY_FAILED]:
      "delivery_failed",

    // Auto-retry lifecycle (Phase 3G):
    [HRM_COMPLIANCE_EXECUTION_AUDIT_ACTIONS.STATUTORY_SUBMISSION_RETRY_ATTEMPTED]:
      "retry_attempted",
    [HRM_COMPLIANCE_EXECUTION_AUDIT_ACTIONS.STATUTORY_SUBMISSION_RETRY_EXHAUSTED]:
      "retry_exhausted",
    ...
    [HRM_COMPLIANCE_EXECUTION_AUDIT_ACTIONS.STATUTORY_SUBMISSION_AGING_DETECTED]:
      "aging_detected",
    [HRM_COMPLIANCE_EXECUTION_AUDIT_ACTIONS.STATUTORY_SUBMISSION_AGING_ESCALATED]:
      "aging_escalated",
    [HRM_COMPLIANCE_EXECUTION_AUDIT_ACTIONS.STATUTORY_SUBMISSION_AGING_CRITICAL]:
      "aging_critical",
 ...
 *     extractors and downstream overview surfaces.
 ...
 *     `@afenda/kernel-execution` / `#features/execution` for the canonical list.
 */
