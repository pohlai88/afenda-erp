...
import {
  listLmsMandatoryComplianceRowsForEmployee,
  summarizeLmsMandatoryComplianceStatuses,
} from "./compliance-lms-bridge.server"
import { deriveComplianceStatusFromLmsMandatoryRow } from "./compliance-lms-bridge.shared"
 ...
 *
 * Consumed by the compliance overview (HRM-CMP-022), the employee detail page
...
export async function getEmployeeComplianceSummary(
  ...
  const [
    ...
    listLmsMandatoryComplianceRowsForEmployee({ organizationId, employeeId }),
  ...
  const lmsTrainingStatuses = lmsMandatoryTraining.map((row) =>
    deriveComplianceStatusFromLmsMandatoryRow(row)
  )
  const lmsTrainingSummary =
    summarizeLmsMandatoryComplianceStatuses(lmsMandatoryTraining)
  ...
  return {
    ...
    mandatoryTrainingCount: mandatoryTraining.length + lmsTrainingSummary.count,
    mandatoryTrainingOverdue:
      trainingStatuses.filter((s) => s === "overdue").length +
      lmsTrainingSummary.overdue,
    mandatoryTrainingAtRisk:
      trainingStatuses.filter((s) => s === "at_risk").length +
      lmsTrainingSummary.atRisk,
    mandatoryTrainingCertExpired:
      trainingStatuses.filter((s) => s === "expired").length +
      lmsTrainingSummary.expired,
