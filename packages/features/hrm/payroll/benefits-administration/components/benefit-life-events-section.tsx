...
import { BenefitLifeEventsTrailingCell } from "./benefit-list-trailing-cells.client"
...
export async function BenefitLifeEventsSection({
  ...
  const t = await getTranslations("Erp.Hrm.benefits.lifeEventsTable")
    ...
    <GovernedPatternCListSection
      ...
      trailingColumn={
        isAdmin
          ? {
              header: t("colActions"),
              Cell: BenefitLifeEventsTrailingCell,
              context: {
                rows: rows.map((row) => ({
                  id: row.id,
