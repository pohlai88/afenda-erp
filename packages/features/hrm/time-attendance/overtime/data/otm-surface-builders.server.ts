...
function buildOtmRequestRows(
  ...
  return rows.map((row) => {
    ...
    return {
      id: row.id,
      ...(copy.orgSlug && includeEmployee
        ? hrmEmployeeListRowLinkFields(copy.orgSlug, row.employeeId, "employee")
        : {}),
      cells,
    }
  })
...
const MY_REQUEST_ACTIONABLE_STATES = new Set(["draft", "submitted", "returned"])
  ...
  return {
    ...
    rows: rows.map((row) => {
      const cells = {
        ...
        timeRange: `${row.startTime}–${row.endTime}`,
        duration: formatOvertimeDurationMinutes(row.durationMinutes),
        dayCategory: otmDayCategoryLabel(
          row.dayCategory,
          copy.dayCategoryLabels
        ),
        state: copy.stateLabelFor(row.state),
        requested: copy.formatRequestedAt(row.requestedAt),
      }
