...
import { hrmEmployee, hrmShiftAssignment } from "@afenda/platform/db/schema"
...
async function resolveSftScheduleUrl(organizationId: string): Promise<string> {
...
export async function notifyRosterPublished(input: {
  ...
  const scheduleUrl = await resolveSftScheduleUrl(input.organizationId)
