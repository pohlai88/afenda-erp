...
/** Leave headroom under route `maxDuration` (60s) for sync watch + teardown. */
const TCI_CRON_SAFE_WALL_MS = 50_000
...
export async function runTimeClockScheduledSyncTick(input?: {
  ...
  const intervalMs = resolveTimeClockSyncIntervalMs(
    process.env[TCI_SCHEDULED_SYNC_INTERVAL_ENV]
  )
  const now = new Date()
  ...
  for (const device of candidates) {
    if (Date.now() - cronStartedAt > TCI_CRON_SAFE_WALL_MS) {
      break
    }
    ...
    try {
      ...
      if (vendorPunches.length > 0) {
        const result = await ingestTimeClockBatch(
          ...
          {
            organizationId: device.organizationId,
            sourceKind: TCI_SCHEDULED_SYNC_SOURCE_KIND,
            deviceId: device.id,
      ...
      logUnexpectedServerError(
        "hrm.time_clock.scheduled_sync.device_failed",
        error,
        {
          deviceId: device.id,
          organizationId: device.organizationId,
