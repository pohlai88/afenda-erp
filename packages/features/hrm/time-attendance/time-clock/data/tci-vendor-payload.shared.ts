...
export type VendorPunchDraft = Omit<
...
export function parseGenericVendorPollPayload(
  ...
  if (!parsed.success) {
    throw new Error(
      "Vendor poll response did not match the generic punch schema."
    )
  }
...
export function parseZebraVendorPollPayload(
  ...
  if (!envelope.success) {
    throw new Error(
      "Zebra vendor response did not match the expected transactions schema."
    )
  }
...
export function parseUkgVendorPollPayload(
  ...
  if (!envelope.success) {
    throw new Error(
      "UKG vendor response did not match the expected punchExports schema."
    )
  }
