...
export const upsertTimeClockDeviceFormSchema = z
  .object({
    ...
    state: z.enum(TCI_DEVICE_REGISTRY_STATES).optional(),
...
export const exportTimeClockReportFormSchema = z
  .object({
    ...
    locationRef: z.string().trim().max(200).optional().nullable(),
    detectionOutcome: z.enum(TCI_DETECTION_OUTCOMES).optional().nullable(),
    syncStatus: z.enum(TCI_DEVICE_SYNC_STATUSES).optional().nullable(),
    rowKinds: z.array(z.enum(TCI_REPORT_ROW_KINDS)).optional(),
...
>
export type TimeClockIngestPunchInput = z.infer<
  typeof timeClockIngestPunchSchema
>
export type TimeClockIngestBatchInput = z.infer<
  typeof timeClockIngestBatchSchema
>
