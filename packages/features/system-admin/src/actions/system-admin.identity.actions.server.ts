...
import type { InviteMemberActionData } from "../contracts";
import { requireSystemAdminIdentityWrite } from "../policies";
import {
  systemAdminInviteMemberActionSchema,
  systemAdminRoleChangeActionSchema,
  systemAdminRoleOverrideActionSchema,
} from "../schemas";
...
export async function changeMemberRoleByInput(input: {
  ...
  const parsed = systemAdminRoleChangeActionSchema.safeParse(input);
  if (!parsed.success) {
    return zodActionFailure(parsed.error);
  }
...
export async function changeMemberRole(formData: FormData) {
  const parsed = systemAdminRoleChangeActionSchema.safeParse({
