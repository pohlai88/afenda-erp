...
import { listAiUsageEvents, listTenantMembers } from "../data-access.server";

export async function getBillingPostureSnapshot(input: {
  organizationId: string;
}) {
  const [aiUsageEvents, lynxEvalRuns, gatewaySpend, members] = await Promise.all([
  ...
  return {
    ...
    gatewaySpendAuthenticationFailed: gatewaySpend.authenticationFailed ?? false,
