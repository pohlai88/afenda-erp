...
export async function createApiCredentialAction(
  ...
  await dispatchSystemAdminWebhook({
    organizationId: organization.id,
    userId: session.id,
    eventType: "tenant.api-credential.created",
    payload: {
      credentialId: result.id,
      keyPrefix: result.keyPrefix,
      scopes,
...
export async function createApiCredentialFormAction(
  _previous:
    | SystemAdminActionResult<CreateApiCredentialActionData>
    | undefined,
  formData: FormData,
) {
...
export async function revokeApiCredentialAction(
  ...
  await dispatchSystemAdminWebhook({
    organizationId: organization.id,
    userId: session.id,
    eventType: "tenant.api-credential.revoked",
...
export async function createWebhookAction(
  ...
  if (invalidEvent) {
    ...
    return systemAdminActionFailure(
      "Select webhook events from the catalog.",
      {
...
export async function upsertSsoConnectionAction(
  ...
  const parsed = ssoSchema.safeParse({
...
export const createWebhookForm = toSystemAdminVoidFormAction(createWebhookAction);
