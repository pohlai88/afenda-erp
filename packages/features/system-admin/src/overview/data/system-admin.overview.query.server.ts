...
export async function getSystemAdminOverview(input: {
  ...
  const [users, invitations, memberships, roles, auditLogs] = await Promise.all([
    listSystemAdminUsers({ organizationId: input.organizationId, limit: 200 }),
    listOrganizationInvitations({ organizationId: input.organizationId, limit: 200 }),
    listSystemAdminMemberships({ organizationId: input.organizationId, limit: 200 }),
    listSystemAdminRoles({ organizationId: input.organizationId }),
    listAuditLogsForOrganization({ organizationId: input.organizationId, limit: 20 }),
  ]);
