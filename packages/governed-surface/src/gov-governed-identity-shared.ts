...
export function toGovernedDomId(prefix: string, key: string) {
...
export function governedHeadingId(kind: string, key: string) {
  return toGovernedDomId(`governed-${kind}`, `${key}-title`);
}

export function governedDescriptionId(kind: string, key: string) {
  return toGovernedDomId(`governed-${kind}`, `${key}-description`);
}

export function governedTestId(kind: string, key: string) {
...
export function governedIdentityAttributes(
  ...
  if (identity.surfaceKey) attrs["data-surface-key"] = identity.surfaceKey;
  if (identity.sectionKey) attrs["data-section-key"] = identity.sectionKey;
  if (identity.componentKey) attrs["data-component-key"] = identity.componentKey;
