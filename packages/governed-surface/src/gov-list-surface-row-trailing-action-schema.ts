 ...
 *
 * Domain builders attach this to each `ListSurfaceRow`. RSC sections register
 * trailing UI via `trailingColumn.cellId` + client `Cell` (never `render`); cells
 * must honor `state` and surface `disabledReason` via `GovernedTrailingActionSlot`.
 */
