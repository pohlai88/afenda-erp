...
/** Runs version migrators before Zod parse on renderer configuration objects. */
export function prepareGovernedConfigurationForParse(raw: unknown): unknown {
  if (raw !== null && typeof raw === "object" && !Array.isArray(raw)) {
    return migrateGovernedConfiguration(raw as Record<string, unknown>)
  }
  return raw
}
