...
import { type MetadataUiPageHeaderInput, parseMetadataUiPageHeader } from "../../schemas/page-header.schema";
...
export function MetadataUiPageHeader({ metadata }: MetadataUiPageHeaderProps) {
    ...
    >
      {header.breadcrumbs.length > 0 ? (
        <nav aria-label="Breadcrumb">
          <ol className={cn("flex flex-wrap items-center", ui.surfaceGap.sm, ui.typography.muted)}>
            {header.breadcrumbs.map((breadcrumb) => (
              <li key={breadcrumb.key} aria-current={breadcrumb.current ? "page" : undefined}>
